/* ══════════════════════════════════════════════════════════════════
   계정 관리 백엔드 (Cloudflare Pages Function) — 어메이징풀빌라 수배서(/dispatch/)
   ...
   경로: POST /api/account   본문(JSON):
     ...
     { action:'find',        email }
   호출 브라우저는 관리자 로그인 상태여야 한다(Authorization: Bearer <access_token>).
   ══════════════════════════════════════════════════════════════════ */
const SUPABASE_URL = 'https://bnzevehxfirjiefwinrh.supabase.co';
const SUPABASE_ANON = 'sb_publishable_lZPUOPT8JPmqSjAHejT8yA_VZqNW58H';
...
// 관리자 판별: metadata.role==='admin' 또는 이메일 아이디가 admin (admin@amazingvilla.app)
function isAdmin(u) {
  ...
  try {
    if (action === 'create') {
      ...
      if (!r.ok) {
        ...
        if (/already|registered|exists|duplicate/i.test(msg)) return json({ error: '이미 등록된 계정(아이디)입니다.' }, 409);
